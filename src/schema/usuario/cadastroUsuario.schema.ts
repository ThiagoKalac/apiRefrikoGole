...
const cadastroUsuarioSchema: yup.ObjectSchema<IUsuarioCriacao> = yup.object().shape({
    ...
    cod_empresa: yup.number().required("Codigo da empresa é obrigatório"),
    credito: yup.number().required("Valor do crédito é obrigatório"),
    codigo_saib: yup.number().required("Codigo da saib é obrigatório"),
