...
const validarTokenLimiter = rateLimit({
    ...
    handler: (req, res) => {
        res.status(429).json({
            mensagem: "Muitas tentativas de validação de token. Tente novamente em 10 minutos.",
        })
    },
});
...
const refreshTokenLimiter = rateLimit({
    ...
    handler: (req, res) => {
        res.status(429).json({
            mensagem: "Muitas tentativas de atualização de token. Tente novamente em 15 minutos.",
        })
    },
  });

export{loginLimiter, validarTokenLimiter, refreshTokenLimiter};