...
const buscarProdutoSaib = async (codigo:number):Promise<IProdutoRespFormatada> => {
    ...
    const respFormatada = tratarRespostaSaib(respSaib)

    return respFormatada;
}


const tratarRespostaSaib = (listaProduto:IProdutoSaibResp[]):IProdutoRespFormatada=>{   
    const empresas:IEmpresaProduto[] = []
    let contador:number = 0

    listaProduto.forEach(produto => {
        const dataValida = validarDataVigencia(produto.DATA_VIGENCIA, produto.DATA_VALIDADE);
        if(!dataValida) {
            contador++
        }
    ...
    const categoria = identificarCategoria(listaProduto[0].COD_MARCA);
  
    return {
        cod: listaProduto[0].COD_PRODUTO,
        nome: formatarNome(listaProduto[0].NOME),
        id_categoria: categoria.idCategoria,
        categoria: categoria.nome,
        litragem: listaProduto[0].LITRAGEM,
        fd_cx: formatarConversor(listaProduto[0].FD_CX, listaProduto[0].FATOR_CX),
        sub_categoria: formartarSubcategoria(listaProduto[0].SABOR),
        empresas
    }
...
const validarDataVigencia = (dataVigencia:Date, dataValidade:Date) : boolean=> {
    if(dataVigencia < dataValidade) return true

    return false
}

const formatarNome = (produtoNome:string):string => {
    ...
    nomeFormatado = nomeFormatado.trim().replace(/\s{2,}/g, ' ');

    return nomeFormatado.toLowerCase();
} 

const identificarCategoria = (idCategoira:number):{idCategoria: number; nome:string} => {
...
const formatarConversor = (unidadeVenda:string, qtd:number):string => {
...
const formartarSubcategoria = (subCategoria:string) => {
...
export {buscarProdutoSaib, ProdutoServico};
